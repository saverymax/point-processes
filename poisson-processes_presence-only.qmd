...
## Diagnostics
Finally, it is possible to look at a variety of NUTS diagnostics metrics. I won't explain these in any detail for now.
```{r}
#| label: nuts-diag
#| eval: true
...
```
neff is the effective sample size estimates the number of independent samples, after accounting for autocorrelation in the chains.
...
I proceed with fitting the model in Stan.
```{r}
#| label: lgcp-fit
#| eval: false
