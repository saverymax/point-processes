...
data = list(N = sites, X = grid_p   ints$aux_x, Z = grid_points$aux_z, y = sim_data$Y[data_reps,], D = distance_mat)
model <- cmdstan_model(model_path)
...
print(p_post)
```
Interestingly, the $\alpha$ and $\beta$ parameters are somewhat identified. We can see that the parameters are nearly recovered, though biased slightly towards 0.
...
print(p)
```
This time, we get realistic estimates of $b$ and $\lambda$, before thinning.

This concludes the analysis using the Poisson Point Process and LGCP.
